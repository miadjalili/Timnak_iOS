...
open class DKCamera: UIViewController, AVCaptureMetadataOutputObjectsDelegate {
   ...
   // open var didFinishCapturingImage: ((_ image: UIImage, _ metadata: [AnyHashable : Any]?) -> Void)?
    ...
    open var cameraOverlayView: UIView? {
        didSet {
            if var cameraOverlayView = cameraOverlayView {
    ...
    open var defaultCaptureDevice = DKCameraDeviceSourceType.front
...
//        }()
       ...
       // self.captureButton = captureButton
        ...
        let IconLive : UIButton = {
            let IconLive = UIButton()
            IconLive.tintColor = .red
            IconLive.setImage(UIImage(systemName: "largecircle.fill.circle",withConfiguration: UIImage.SymbolConfiguration(pointSize: 30, weight: .medium)), for: .normal)
            return IconLive
        }()
            
    
       IconLive.frame = CGRect(x: 345, y: 15, width: 30, height: 30)
            //CGPoint(x: 345, y: 15 )
       contentView.addSubview(IconLive)
        
        
        let minimizebutton : UIButton = {
        let minimizebutton = UIButton()
            //minus.square
            minimizebutton.setImage(UIImage(systemName: "minus.square",withConfiguration: UIImage.SymbolConfiguration(pointSize: 30, weight: .light)), for: .normal)
        minimizebutton.addTarget(self, action: #selector(DKCamera.miniview), for: .touchUpInside)
            return minimizebutton

        }()
        minimizebutton.tintColor = .white
         minimizebutton.frame = CGRect(x:  250, y: 250, width: 30, height: 30)
        ...
        cancelButton.frame.origin = CGPoint(x: contentView.bounds.width - cancelButton.bounds.width - 290, y: 250 )
